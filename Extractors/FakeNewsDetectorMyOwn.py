...
filepath = "/home/tacticrabbit/MyOwnFakeAndReal.csv"
myOwn_data = pd.read_csv(filepath, sep=',')
myOwn_data.columns = [
'id','headline','text', 'category', 'label']

myOwn_data = myOwn_data.dropna(subset=['text']).reset_index(drop=True)


# Filter for Fake samples only (SAMPLE FAKE (0) AND REAL (1))
sample_fake = myOwn_data[myOwn_data['label'] == 'fake'].sample(20)
...
fake_articles = myOwn_data[myOwn_data["label"] == 'fake']["text"].astype(str).tolist()
# this gets real articles from my own dataset
real_articles = myOwn_data[myOwn_data["label"] == 'real']["text"].astype(str).tolist()
...
# save to CSV file
pattern_confidence.to_csv("fake_newsMyOwn.csv", index=False)
