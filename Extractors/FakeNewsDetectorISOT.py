...
sample_fake = isot_dataFAKE.dropna(subset=['text']).sample(20).reset_index(drop=True)
...
real_articles = isot_dataTRUE["text"].dropna().astype(str).tolist()
fake_articles = isot_dataFAKE["text"].dropna().astype(str).tolist()
...
# save to CSV file
pattern_confidence.to_csv("fake_news_patterns_confidenceISOTDATASET.csv", index=False)
