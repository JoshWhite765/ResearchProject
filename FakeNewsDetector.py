...
hf_data.columns = ['unnamed', 'title', 'text', 'subject', 'date', 'label']
hf_news = hf_data.dropna(subset=['text']).reset_index(drop=True)


# Filter for Fake samples only (SAMPLE FAKE (0) AND REAL (1))
sample_fake = hf_news[hf_news['label'] == 0].sample(20)
...
fake_articles = hf_news[hf_news["label"] == 0]["text"].astype(str).tolist()
real_articles = hf_news[hf_news["label"] == 1]["text"].astype(str).tolist()
...
# save to CSV file
pattern_confidence.to_csv("fake_news_patterns_confidenceHFDATASET.csv", index=False)
